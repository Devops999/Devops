-- begin BICYCLEAPPS2_CLIENT
create table BICYCLEAPPS2_CLIENT (
    ID varchar(36) not null,
    VERSION integer not null,
    CREATE_TS timestamp,
    CREATED_BY varchar(50),
    UPDATE_TS timestamp,
    UPDATED_BY varchar(50),
    DELETE_TS timestamp,
    DELETED_BY varchar(50),
    --
    NAME varchar(255) not null,
    PHONE_NUMBER varchar(255) not null,
    EMAIL varchar(255) not null,
    --
    primary key (ID)
)^
-- end BICYCLEAPPS2_CLIENT
-- begin BICYCLEAPPS2_MECHANIC
create table BICYCLEAPPS2_MECHANIC (
    ID varchar(36) not null,
    VERSION integer not null,
    CREATE_TS timestamp,
    CREATED_BY varchar(50),
    UPDATE_TS timestamp,
    UPDATED_BY varchar(50),
    DELETE_TS timestamp,
    DELETED_BY varchar(50),
    --
    USER_ID varchar(36) not null,
    HOURLY_RATE decimal(19, 2) not null,
    --
    primary key (ID)
)^
-- end BICYCLEAPPS2_MECHANIC
-- begin BICYCLEAPPS2_SPARE_PART
create table BICYCLEAPPS2_SPARE_PART (
    ID varchar(36) not null,
    VERSION integer not null,
    CREATE_TS timestamp,
    CREATED_BY varchar(50),
    UPDATE_TS timestamp,
    UPDATED_BY varchar(50),
    DELETE_TS timestamp,
    DELETED_BY varchar(50),
    --
    TITLE varchar(255) not null,
    DESCRIPTION longvarchar,
    PRICE decimal(19, 2) not null,
    --
    primary key (ID)
)^
-- end BICYCLEAPPS2_SPARE_PART
-- begin BICYCLEAPPS2_ORDER
create table BICYCLEAPPS2_ORDER (
    ID varchar(36) not null,
    VERSION integer not null,
    CREATE_TS timestamp,
    CREATED_BY varchar(50),
    UPDATE_TS timestamp,
    UPDATED_BY varchar(50),
    DELETE_TS timestamp,
    DELETED_BY varchar(50),
    --
    CLIENT_ID varchar(36) not null,
    MECHANIC_ID varchar(36) not null,
    DESCRIPTION longvarchar,
    HOUR_SPENT integer,
    AMOUNT decimal(19, 2) not null,
    STATUS integer not null,
    PALVISHA integer,
    ANMOL integer,
    --
    primary key (ID)
)^
-- end BICYCLEAPPS2_ORDER
-- begin BICYCLEAPPS2_ORDER_SPARE_PART_LINK
create table BICYCLEAPPS2_ORDER_SPARE_PART_LINK (
    ORDER_ID varchar(36) not null,
    SPARE_PART_ID varchar(36) not null,
    primary key (ORDER_ID, SPARE_PART_ID)
)^
-- end BICYCLEAPPS2_ORDER_SPARE_PART_LINK
